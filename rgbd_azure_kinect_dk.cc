    ...
    k4a_image_t *transformed_image)
{
  if (K4A_RESULT_SUCCEEDED !=
      k4a_transformation_depth_image_to_color_camera(
          ...
          *transformed_image))
  {
    std::cerr << "Failed to compute transformed depth image for k4a" << std::endl;
    return false;
  }
...
void init_memory()
{
  ...
  {
    std::cerr << "WARNING: Failed to create transformed depth image!" << std::endl;
  }
...
k4a_depth_mode_t k4a_convert_string_to_mode(const std::string strmode)
{
...
int main(int argc, char **argv)
{
  ...
  {
    std::cerr << std::endl
              << "Usage: ./rgbd_azure_kinect_dk path_to_vocabulary path_to_settings (trajectory_file_name)"
              << std::endl;
    return 1;
  }
  ...
  node = fsSettings["Kinect.depthmode"];
  if (!node.empty() && node.isString())
  {
    depthmode = node.string();
  }
  ...
  config.depth_mode = k4a_convert_string_to_mode(depthmode);
 ...
 */
  ...
  while (!SLAM.isShutDown())
  {
    ...
    SLAM.TrackRGBD(cv_undistorted_color, cv_undistorted_depth, timestamp);
